...
**_Note: other solutions are possible, map points accordingly._**
...
**_Grading: 1 pt for each stack (all or nothing), 2 pts for the answer (also all or nothing)_**
...
The array below shows the storage of a **Min-Heap** in the **_middle_** of an insert operation.
...
Picture of the heap as a balanced binary tree **_after_** insertion is complete:
...
**_Grading: 1 pt for (1,3,2), 2 pts for (8, 5, 7), 1 pt for (11, 10, 12). If lots incorrect but heap property maintained, 2 pts total, If heap property isn't maintained 0 pts total_**
...
(a) (8 pts) An AVL tree stores the grades of the class (in between 1 and 100 inclusive). Create an AVL tree by inserting the following values into an initially empty AVL Tree in the order given: 98, 87, 92, 81, 75, 68, and 100. Show the state of the tree **_after_** each insertion and draw a box around each of these results.

(b) (2 pts) What is the fewest and most number of comparisons for looking for a valid grade that is **_not_** within this tree?

Fewest number of comparisons = __________

Most number of comparisons = ____________
...
**_Grading: 1 pt per correct tree. 1 pt if last tree is a valid AVL with all 7 elements_**
...
1. Merging a sorted array of size **_m_** with a sorted array of size **_n_** into one sorted array. 
1. Creating a heap out of **_n_** unsorted integers.
1. **Worst case** run-time of running a Quick Sort on **_n_** integers.
1. Inserting an element to the front of a linked list with **_n_** elements.
1. Deleting **_m_** items, one by one, from an **AVL** tree which originally contains **_n_** items (n ≥ **_m_**)
1. A sequence of **_p_** push operations onto a stack that originally had **_n_** elements on it. (Assume the stack has enough space to handle the sequence of push operations.)
1. **Average case** run time of an insertion sort on **_n_** unsorted integers.
1. Calculating $$a^b \bmod c$$, using fast modular exponentiation, assuming that each multiply and each mod operation take $$O(1)$$ time.
1. Pre-order traversal of a binary tree with height **_h_** and **_n_** nodes.
1. **Worst case** run-time for searching for an element in a binary search tree with **_n_** nodes.

{% endtab %}

{% tab title="Solution" %}

1. Merging a sorted array of size **_m_** with a sorted array of size **_n_** into one sorted array. $$O(m + n)$$
1. Creating a heap out of **_n_** unsorted integers. $$O(n)$$
1. **Worst case** run-time of running a Quick Sort on **_n_** integers. $$O(n^2)$$
1. Inserting an element to the front of a linked list with **_n_** elements. $$O(1)$$
1. Deleting **_m_** items, one by one, from an **AVL** tree which originally contains **_n_** items (n ≥ **_m_**) $$O(m\log{n})$$
1. A sequence of **_p_** push operations onto a stack that originally had **_n_** elements on it. (Assume the stack has enough space to handle the sequence of push operations.) $$O(p)$$
1. **Average case** run time of an insertion sort on **_n_** unsorted integers. $$O(n^2)$$
1. Calculating $$a^b \bmod c$$, using fast modular exponentiation, assuming that each multiply and each mod operation take $$O(1)$$ time. $$O(\log{b})$$
1. Pre-order traversal of a binary tree with height **_h_** and **_n_** nodes. $$O(n)$$
1. **Worst case** run-time for searching for an element in a binary search tree with **_n_** nodes. $$O(n)$$
...
An algorithm to process input data about n cities takes O(n!) time. For n = 10, the algorithm runs in 10 milliseconds. How many **_seconds_** should the algorithm take to run for an input size of n = 12? Put a box around your final answer.
...
T(12) = c(12!)
\\
\cfrac{10ms}{10!} x 12! = \cfrac{10ms}{10!} × 10! × 11 × 12 = 10ms × (132) = 1320ms = 1.32sec
$$

**_Grading: 2 pts solving for c, 2 pts for plugging 12 and canceling to get to 1320 ms, 1 pt to answer in seconds._**
...
**_Grading: 2 pts for iteration with T(n-2), 2 pts for iteration with T(n-3), 2 pts for general guess after k steps. 1 pt for plugging in k = n (or k = n-1), 3 pts for simplifying that to the final answer._**
...
1. An array, **_values_**, storing the original sequence
1. An integer, **_k_**, representing that we want to only consider the values in the array upto index k, including it.
1. An integer, **_max_**, representing the maximum value allowed in the increasing sequence.

Our recursive function will return the length of the longest increasing subsequence of **_values_**[0..**_k_**] such that no value in the increasing subsequence exceeds max.

Complete the implementation of this **_recursive_** function below:
