...
Define the weighted sum of an integer array a[0], a[1], ..., a[n-1] to be $$\sum_{i=1}^{n} (ia[i - 1])$$. For example, the weighted sum of the array [7, 5, 8] would be 1 × 7 + 2 × 5 + 3 × 8 = 41. Write a **_recursive_** function that takes in an array `numbers` and its length `n`, and returns its weighted sum. You may assume that there will be no issues with integer overflow.
...
**_Grading: 1 pt per row, row has to be perfect to earn the point._**
